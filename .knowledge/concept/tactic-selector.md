...
model:
  ...
  - tactic conditions are data:derived-predicate over concept:sight, so switching is deterministic and replays exactly
  - static code: tactics compile with the tree per decision:behavior-tree-compiled-to-go; nothing is rebuilt at runtime
player_orders: a tactic order to an allied agent is an ordinary concept:action — it enters concept:world-state, is projected into concept:sight, and the selector reads it there, so decision:no-ai-game-mode holds
...
recorded: the active tactic at each decision is visible in data:decision-record via the sight, so ui:chip-benchmark can chart tactic frequency
```
