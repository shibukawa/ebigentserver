...
implementation: the visibility predicate of rule:sight-content-owned-by-game, materialized by concept:agent-view
annotation: each projection declares itself in data:visibility-annotation
security_note: scopes are a boundary between players, not a display filter; hidden data is never sent, see policy:sight-scoped-information
```
