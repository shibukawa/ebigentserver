...
```yaml
problem: an sight carries enemy position and own position; a useful condition needs in_range, which no field states
fields:
...
constraints: rule:generated-agent-code-is-deterministic and the sight only limit of rule:analysis-restricted-to-visible-fields
```
