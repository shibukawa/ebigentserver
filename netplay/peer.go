...
func (p *Peer[S, A, D]) sendState(ctx context.Context, tick session.Tick, world *S) {
	p.mu.Lock()
...
func (p *Peer[S, A, D]) Run(ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
...
func (p *Peer[S, A, D]) absorbDatagram(payload []byte) {
...
// duration.
func (p *Peer[S, A, D]) takeToken() bool {
	now := time.Now()
...
func (p *Peer[S, A, D]) violate(reason string) {
	p.violations.Add(1)
...
func (p *Peer[S, A, D]) depart(reason string) {
	p.departed.Do(func() {
		p.gone.Store(true)
