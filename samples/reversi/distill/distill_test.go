...
func TestGeneratedSourcesAreCurrent(t *testing.T) {
	...
	if string(a) != string(b) {
		t.Fatal("chips.json is stale; rerun: go test ./samples/reversi/distill -update")
	}
}
...
// against them:
//
//	go test ./samples/reversi/distill -update
//
...
func current(t *testing.T, path string, generated []byte) {
	...
	if string(generated) != string(committed) {
		t.Fatalf("%s is stale; rerun: go test ./samples/reversi/distill -update", path)
	}
}
