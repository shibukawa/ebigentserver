...
func TestHybridExchangeOverTheWire(t *testing.T) {
	...
	s, err = session.New(session.Config[rtslite.State, rtslite.Input, rtslite.Observation]{
		...
		Game:      rtslite.Game{Players: 2, TickLimit: 3600},
	...
	clients := map[session.SlotID]*netplay.Client[msg.PlayerView, rtslite.Input, msg.PlayerViewDelta, rtslite.Observation]{}
	var mu sync.Mutex
	for _, slot := range rtslite.Slots(2) {
		f := faults
		f.Seed += uint64(slot)
