...
// TickGame extends Game with a per-tick simulation step for realtime play
...
type TickGame[S, A, O any] interface {
	Game[S, A, O]
...
const (
	...
	// runs, tests, and replays (concept:simulation-mode).
	Unlimited
)
...
// (operator stop; unfinished slots end Abandoned). The configured Game
// must implement TickGame and Config.Tuning must be declared.
//
...
func (s *Session[S, A, O]) RunRealtime(ctx context.Context, tc TimeControl) error {
	game, ok := s.cfg.Game.(TickGame[S, A, O])
	if !ok {
		return fmt.Errorf("session: RunRealtime requires the game to implement TickGame")
	}
	if s.cfg.Tuning == nil {
