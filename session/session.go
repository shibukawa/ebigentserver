...
type Config[S, A, O any] struct {
	...
	// passed to StageRuleSet.Start and recorded in the episode header.
	Seed uint64
...
func (s *Session[S, A, O]) Run(ctx context.Context) error {
	...
	s.world = s.cfg.RuleSet.Start(s.cfg.Seed)
	...
	for {
		...
		acting := slices.Clone(s.cfg.RuleSet.ActingSlots(&s.world))
		if len(acting) == 0 {
		...
		for _, slot := range acting {
			...
			s.cfg.RuleSet.Apply(&s.world, slot, action)
		}
...
func (s *Session[S, A, O]) evaluateAll() (map[SlotID]EvaluationSignal, bool) {
	...
	for _, slot := range s.slots {
		sig := s.cfg.RuleSet.Evaluate(&s.world, slot)
		signals[slot] = sig
