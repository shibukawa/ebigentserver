...
// askMove is the action position of a rule set: the whole value in the
// array shape.
func askMove(dst []byte, v Move) []byte { return cborbind.AppendCBORInArrayTo(dst, v) }

func askDecodeMove(data []byte) (Move, error) { return cborbind.DecodeCBORInArrayFrom[Move](data) }

// askTTTWorld is the world position of a rule set: the whole value in the
// map shape.
