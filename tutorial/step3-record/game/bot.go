...
// NewBot is the factory run.Binding.NewAgent takes, and the one line
// bind.go names.
//
// The id names the policy rather than the seat. It is what the lobby
// shows, and once there are two kinds of bot it is what tells their rows
// apart in a corpus — the agent_kind column of a recorded decision.
func NewBot(session.SlotID) (string, session.Agent[Sight, msg.Move]) {
	return "tactic", &Bot{}
}
