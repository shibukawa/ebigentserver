...
func TestYourOwnPlayIsWhatGetsRecorded(t *testing.T) {
	...
	if err := roster.FillBots(game.HandWritten); err != nil {
		t.Fatal(err)
	}
...
// factory the lobby's press uses, and the run finishes in the time it
// takes to open the files.
//
...
func recordHeadless(t *testing.T, matches int) string {
	...
	binding.NewAgent = game.HandWritten
	err := run.Serve(context.Background(), game.Options(), binding, run.ServeOptions{
		Matches: matches,
		Seed:    1,
